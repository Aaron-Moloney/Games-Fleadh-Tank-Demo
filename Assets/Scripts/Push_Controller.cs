...
public class Push_Controller : MonoBehaviour
{
    ...
    void OnCollisionEnter()
    {
        ...
        {
            Debug.Log("Hit");
            Instantiate(boom,transform.position + (transform.position - playerRef.transform.position).normalized * 15,transform.rotation);
            Destroy(gameObject);
        }
        else
        {

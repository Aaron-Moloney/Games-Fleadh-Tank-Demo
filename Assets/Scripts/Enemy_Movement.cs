...
public class Enemy_Movement : MonoBehaviour
{
    ...
    void Update()
    {
        if (health <= 1)
        {
            if (Random.Range(0,20) == 1 ) Instantiate(prize,new UnityEngine.Vector3(transform.position.x,20f,transform.position.z),transform.rotation);
    ...
    void OnTriggerEnter(Collider collision)
    {
        ...
        if (iFrame)
        {
            ...
            if (collision.gameObject.CompareTag("PushExpl"))
            {
                ...
                UnityEngine.Vector3 explForce = (this.transform.position - playerRef.transform.position).normalized*1850;
                explForce.y = 250;
                rb.AddForce(explForce);
            }
